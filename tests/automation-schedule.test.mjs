...
test('uses a ten-minute Cloudflare clock with a staggered GitHub fallback', () => {
  ...
  assert.match(cloudflareWorker, /cron === '45 19,20 \* \* \*'/u);
...
test('serializes every Pages deployment and persists reconciled source queue state', () => {
  ...
  assert.match(workflow, /test ! -f data\/aliexpress-source-diagnostics\.json \|\| git add data\/aliexpress-source-diagnostics\.json/u);
