...
export default async function GuidePage({ params }: GuidePageProps) {
  ...
  const guide = getGuide(guia);
  if (!guide) notFound();
  const updatedAt = "updatedAt" in guide ? guide.updatedAt : "2026-08-25";
  const readingMinutes = "readingMinutes" in guide ? guide.readingMinutes : 3;
  const deals = publishedDeals.filter(guide.matches).slice(0, 3);
  const articleSchema = { "@context": "https://schema.org", "@type": "Article", headline: guide.title, description: guide.description, inLanguage: "es-ES", datePublished: "2026-08-12", dateModified: updatedAt, mainEntityOfPage: `${siteUrl}/guias/${guia}/`, author: { "@type": "Organization", name: "Chollos al Dia" }, publisher: { "@type": "Organization", name: "Chollos al Dia", url: siteUrl } };
  ...
  return <main className="guidePage">
    ...
    <article className="guideArticle shell">
      ...
      <header className="guideHero"><p className="eyebrow"><span aria-hidden="true" />{guide.eyebrow}</p><h1>{guide.title}</h1><p>{guide.intro}</p><div><span>Lectura: {readingMinutes} min</span><span>Actualizada el <time dateTime={updatedAt}>{new Date(`${updatedAt}T12:00:00Z`).toLocaleDateString("es-ES", { day: "numeric", month: "long", year: "numeric" })}</time></span><span>Por Chollos al Dia</span></div></header>
