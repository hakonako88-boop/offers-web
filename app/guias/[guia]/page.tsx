...
export default async function GuidePage({ params }: GuidePageProps) {
  ...
  return <main className="guidePage">
    ...
    <article className="guideArticle shell">
      ...
      {deals.length > 0 && <section className="guideDeals" aria-labelledby="guide-deals-title"><div className="sectionIntro"><div><p className="eyebrow"><span aria-hidden="true" />SELECCION ACTUAL</p><h2 id="guide-deals-title">Ofertas para aplicar esta guia.</h2></div><p>Datos registrados al publicar. Comprueba siempre la ficha final de la tienda.</p></div><div className="dealGrid storeDealGrid">{deals.map((deal) => { const discount = dealDiscount(deal); const savings = dealSavings(deal); return <article className="dealCard" key={deal.id}><Link className="imageWrap dealPreviewLink" href={dealHref(deal.id)} aria-label={`Ver oferta: ${deal.title}`}><img src={deal.imageUrl} alt={deal.title} width={720} height={560} />{discount > 0 && <span className="discountBadge">-{discount}%</span>}<span className="storeBadge">{deal.store}</span></Link><div className="dealBody"><p className="categoryLabel">{deal.category}</p><h3><Link href={dealHref(deal.id)}>{deal.title}</Link></h3><div className="priceRow"><strong>{money.format(deal.price)}</strong>{discount > 0 && <span>Antes <s>{money.format(deal.oldPrice)}</s></span>}</div>{savings > 0 && <p className="saving">Ahorras {money.format(savings)}</p>}<Link className="dealButton" href={dealHref(deal.id)}>Ver oferta y analisis <span aria-hidden="true">→</span></Link></div></article>; })}</div></section>}
