...
export default async function CategoryOffersPage({ params }: CategoryPageProps) {
  ...
  const schema = {
    "@context": "https://schema.org", "@type": "CollectionPage", "@id": `${siteUrl}/chollos/${categoria}`,
    name: category.seoTitle, description: category.description, inLanguage: "es-ES", isPartOf: { "@id": `${siteUrl}/#website` },
    mainEntity: { "@type": "ItemList", numberOfItems: deals.length, itemListElement: deals.map((deal, index) => ({ "@type": "ListItem", position: index + 1, url: `${siteUrl}${dealHref(deal.id)}`, name: deal.title, image: absoluteImageUrl(deal.imageUrl) })) },
  };
