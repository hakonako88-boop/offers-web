...
const rules: Rule[] = [
  ...
  { category: "Belleza y cuidado personal", subcategory: "Cuidado personal", pattern: /\b(?:crema\s+hidratante|anti[- ]?brillo|cuidado\s+facial|men\s+expert|l['’]?or[eé]al|maybelline|oral-b|recortadora\s+(?:de\s+)?(?:nariz|orejas))\b/iu, weight: 1 },
  ...
  { category: "Alimentación", pattern: /\b(?:caf[eé]\s+(?:en\s+grano|molido)|chocolate|galleta|aceite\s+de\s+oliva|conserva|bebida|snack|alimentaci[oó]n|yatekomo|fideos?|arroz|pasta\s+(?:alimenticia|italiana)|salsa|caldo|cereales?|lomo\s+embuchado|jam[oó]n\s+(?:ib[eé]rico|serrano)|cerveza|lager|vino|refresco|leche\s+(?:entera|semi|desnatada))\b/iu, weight: .98 },
  ...
  { category: "Hogar", pattern: /\b(?:hogar|mueble|colch[oó]n|almohada|coj[ií]n|s[aá]bana|toalla|limpieza|jard[ií]n|bancal|piscina|l[aá]mpara|iluminaci[oó]n|cama\s+para\s+perro|mascota|ventilador\s+de\s+techo|cesto\s+(?:de\s+)?ropa|cesta\s+(?:de\s+)?lavander[ií]a|organizador\s+(?:de\s+)?cocina|soporte\s+(?:para\s+)?estropajo)\b/iu, weight: .96 },
